...
class CosplayfuzhuangPipeline(object):
    def open_spider(self,spider):
        #打开文件
        self.fp = open('cosplay.text','w',encoding='utf8')
...
class MysqlPipeline(object):
    ...
    def process_item(self,item,spider):
        # sql = 'insert into fz_goodsdet(goodsname,goodsprice,goodscon,goodsimg,goodst_id) values("%s","%s","%s","%s","%s")' % (item['goodsname'],item['goodsprice'],item['goodscon'],item['goodsimg'],item['goodst_id'])
